...
def gao(category, metadata=None, reviews=None, K=5, st_year=2017, st_month=10, ed_year=2018, ed_month=11, output=True):
    ...
    while True:
        ...
        if st_year > 1996 and len(items) < 5000:
            break
        if not flag:
            break
    ...
    if st_year > 1996 and len(items) < 5000:
        if st_month - 3 <= 0:
            st_month = st_month + 12 - 3
            st_year -= 1
        else:
            st_month -= 3
        gao(category, metadata = metadata, reviews = reviews, K=K, st_year=st_year, st_month=st_month, ed_year=ed_year, ed_month=ed_month, output=True)
        return
    ...
    if not os.path.exists("./data/info"):
        os.mkdir("./data/info")
    with open(f"./data/info/{category}_{K}_{st_year}-{st_month}-{ed_year}-{ed_month}.txt", 'w') as f:
        for item in items:
            item2id[item] = count
    ...
    if not os.path.exists("./data/test"):
        os.mkdir("./data/test")
    with open(f"./data/train/{category}_{K}_{st_year}-{st_month}-{ed_year}-{ed_month}.csv", "w") as f:
        writer = csv.writer(f)
    ...
    with open(f"./data/valid/{category}_{K}_{st_year}-{st_month}-{ed_year}-{ed_month}.csv", "w") as f:
        writer = csv.writer(f)
        writer.writerow(['user_id', 'item_asins', 'item_asin', 'history_item_id', 'item_id', 'history_item_title', 'item_title', 'history_rating', 'rating', 'history_timestamp', 'timestamp'])
        writer.writerows(interaction_list[int(len(interaction_list) * 0.8):int(len(interaction_list) * 0.9)])
    with open(f"./data/test/{category}_{K}_{st_year}-{st_month}-{ed_year}-{ed_month}.csv", "w") as f:
        writer = csv.writer(f)
        writer.writerow(['user_id', 'item_asins', 'item_asin', 'history_item_id', 'item_id', 'history_item_title', 'item_title', 'history_rating', 'rating', 'history_timestamp', 'timestamp'])
